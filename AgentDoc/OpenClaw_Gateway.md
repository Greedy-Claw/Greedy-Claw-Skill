...
`server`, `features`, `snapshot`, and `policy` are all required by the schema (`src/gateway/protocol/schema/frames.ts`). `canvasHostUrl` is optional. `auth` reports the negotiated role/scopes when available, and includes `deviceToken` when the gateway issues one. When no device token is issued, `hello-ok.auth` can still report the negotiated permissions:
...
## [Common RPC method families](#common-rpc-method-families)
...
### [System and identity](#system-and-identity)
...
### [Models and usage](#models-and-usage)
...
### [Channels and login helpers](#channels-and-login-helpers)
...
### [Messaging and logs](#messaging-and-logs)
...
### [Secrets, config, update, and wizard](#secrets-config-update-and-wizard)
...
### [Existing major families](#existing-major-families)
#### [Agent and workspace helpers](#agent-and-workspace-helpers)
...
#### [Device pairing and device tokens](#device-pairing-and-device-tokens)
...
#### [Node pairing, invoke, and pending work](#node-pairing-invoke-and-pending-work)
...
#### [Approval families](#approval-families)
...
#### [Other major families](#other-major-families)
...
### [Common event families](#common-event-families)
...
### [Operator helper methods](#operator-helper-methods)
...
## [Agent delivery fallback](#agent-delivery-fallback)
...
### [Device auth migration diagnostics](#device-auth-migration-diagnostics)
